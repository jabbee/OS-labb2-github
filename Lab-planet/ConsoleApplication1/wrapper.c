...
#define TIMERID			10  /* id for timer that is used by the thread that manages the window where graphics is drawn */
...
HANDLE mailslotConnect (LPTSTR lpszSlotName, smessage meddelande) {
	...
	mailslotWrite(hFile, &meddelande, sizeof(meddelande.msg));
...
int mailslotWrite(HANDLE mailSlot, smessage* msg, int msgSize) {
	...
	fResult = WriteFile(mailSlot, msg->msg, msgSize, &cbWritten, (LPOVERLAPPED)NULL);
...
DWORD mailslotRead (HANDLE mailbox, planet_type *msg, int msgSize) {
	...
	while (cMessage != 0)  // retrieve all messages
	{
		...
		//planetPtr = (planet_type*)malloc(sizeof(planet_type));
		planetPtr = (planet_type*)GlobalAlloc(GPTR, cbMessage);
		if (lpszBuffer == NULL)
			return FALSE;
		lpszBuffer[0] = '\0';
		...
		fResult = ReadFile(mailbox, planetPtr, cbMessage, &cbRead, &ov);
		*msg = *planetPtr;
		//planetPtr = (planet_type*)lpszBuffer;
		if (!fResult)
		{
